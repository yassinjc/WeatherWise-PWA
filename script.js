...
function getCityWeather(url) {
 ...
 <div class="weather__summary">
    ...
    <li><i class="wi wi-raindrop"></i>  <span>${
      data.main.humidity
    }%</span></li>
    <li><i class="wi wi-strong-wind"></i> <span>${
      data.wind.speed
    } m/s</span></li>
    </ul>
 </div>
...
function getWeatherByCoordinates(latitude, longitude) {
  getCityWeather(
    `https://api.openweathermap.org/data/2.5/weather?lat=${latitude}&lon=${longitude}&APPID=9bc387c4218b073bbfc9a156235d3ba0&units=metric`
  );
}

function getForecastByCoordinates(latitude, longitude) {
  getForecast(
    `https://api.openweathermap.org/data/2.5/forecast?lat=${latitude}&lon=${longitude}&APPID=9bc387c4218b073bbfc9a156235d3ba0&units=metric`
  );
}

function getWeatherByCity(city) {
  getCityWeather(
    `https://api.openweathermap.org/data/2.5/weather?q=${city}&APPID=9bc387c4218b073bbfc9a156235d3ba0&units=metric`
  );
}

function getForecastByCity(city) {
  getForecast(
    `https://api.openweathermap.org/data/2.5/forecast?q=${city}&APPID=9bc387c4218b073bbfc9a156235d3ba0&units=metric`
  );
}
...
searchBtn.addEventListener("click", (e) => {
  ...
  document.querySelector(".graph__img").removeAttribute("hidden");
